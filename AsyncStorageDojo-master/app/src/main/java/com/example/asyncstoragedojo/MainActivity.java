            ...
            handler.post(() -> {
                ...
                if (!jsonResponse.isEmpty()) {
                    try {
                        ...
                        output.append("Station: ").append(hydroData.station.name).append("\n\n");

                        for (Observation obs : hydroData.observations) {
                            output.append("Time: ").append(obs.observationTimeUtc).append("\n");
                            output.append("Water level: ").append(obs.waterLevel).append("\n");
                            output.append("Water temperature: ").append(obs.waterTemperature).append("\n");
                            output.append("---------------------------\n");
                        }
    ...
    private HydroData parseHydroData(String jsonString) throws JSONException {
        ...
        for (int i = 0; i < observationsJson.length(); i++) {
            ...
            Observation observation = new Observation(
                    ...
                    obsJson.optDouble("waterLevel", Double.NaN),
